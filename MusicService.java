/**
 * Foreground музыкальный сервис.
 * Жизненный цикл:
 * - onCreate() создает MediaPlayer, уведомление, подписывается на телефонные звонки.
 * - onStartCommand() обрабатывает действия из уведомления (play/pause, next, prev).
 * - onBind() возвращает Binder для связи с Activity (через ViewModel).
 * - onUnbind() вызывается, когда последний клиент отвязывается – сервис продолжает работать,
 *   если проигрывается музыка, иначе может быть остановлен системой.
 * - onDestroy() освобождает MediaPlayer и уведомление.
 *
 * Сервис запущен в foreground с типом mediaPlayback, что позволяет ему работать даже
 * при закрытом приложении. Управление через уведомление обеспечивает работу на
